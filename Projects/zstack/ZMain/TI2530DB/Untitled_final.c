...
uint8 count = 0,count1 = 0,count3 = 0;
uint8 led_blink_t =0,led_no_t = 0,led_no_toggle=0,T_prev=1,timer=1,led_toggle_t=0;
...
void delete_scene_slected(uint8 arr_index);
...
void delete_scene(void){
  ...
  arr_index = search_scn_id(scn_id);
    ...
    if(arr_index>=0){
      delete_scene_slected(arr_index);
      uart_send_ack(cmd_id);
    }
    else
    {
...
void uartRxCb( uint8 port, uint8 event ) {
   ...
   while (Hal_UART_RxBufLen(port))
 {
   ...
   if(ch != 0xAA && uart_cmd_started == 1){
     data_length = ch;
     uart_cmd_started=2;
   }

   if(uart_cmd_started == 2 ){
     if(data_ptr < data_length){
          uart_data[ data_ptr++ ] = ch;
     }
    if(data_ptr >= data_length && ch == 0xFF){
      uart_cmd_end = 1;
      uart_cmd_started = 0;
      data_ptr =0;
    }
