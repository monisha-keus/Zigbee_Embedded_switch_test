...
uint8 uart_data[10] = {'0'};
uint8  rec_buff[5] = {'0'};
uint8 scene_arr[3][10];
uint8 uart_cmd_started = 0,uart_cmd_end = 0;
uint8 data_length,cmd_id,task_type,scn_no=0;

KeusGPIOPin ledPin1 = {0, 0, GPIO_OUTPUT, false, GPIO_LOW};
KeusGPIOPin ledPin2 = {0, 1, GPIO_OUTPUT, false, GPIO_LOW};
KeusGPIOPin ledPin3 = {0, 4, GPIO_OUTPUT, false, GPIO_LOW};
KeusGPIOPin ledPin4 = {0, 5, GPIO_OUTPUT, false, GPIO_LOW};
...
/*
** This function is to decrypt the received data and perform the corresponding data
*/
void decrypt_data(uint8 *uart_data)
{
  uint8 data_ptr = 0,rec_ptr = 0;
  data_length = uart_data[ data_ptr + 1 ]; 

  //uint8 length = (uint8)length_s;
  data_ptr = data_ptr + 2;
...
/*
* **This function is for processed the data received from uart
*/
...
void save_scene_task(void){
  uint8 i=0;
  uint8 no_of_led = rec_buff[0];
  for(i=0;i<data_length;i++){
    scene_arr[scn_no][i] = rec_buff[i];
  }
  scn_no++;
